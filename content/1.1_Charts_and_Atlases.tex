...
\begin{document}
    ...
    \begin{definition}
        ...
        \end{equation*}
        is biholomorphic. A \uldef{$d$-dimensional complex atlas on $X$} is a collection $\mc{A}\coloneqq\l\{\l(U_i,\phi_i\r)\r\}_{i\in I}$ of $d$-dimensional complex charts such that every two charts $\l(U_i,\phi_i\r)$ and $\l(U_j,\phi_j\r)$ are holomorphically compatible and $X=\bigcup_{i\in I}U_i$.
    \end{definition}
    ...
    \begin{remark}
        Two atlases $\mc{A}$ and $\mc{B}$ on a manifold $X$ are said to be \ul{analytically equivalent} if every chart in $\mc{A}$ is compatible with every chart in $\mc{B}$. By Zorn's Lemma, every atlas $\mc{A}$ of a manifold $X$ is contained in a unique maximal atlas $\mf{U}$ on $X$. Moreover, two atlases are equivalent iff they are contained in the same maximal atlas, which justifies the following definition.\exqed
    \end{remark}
    \begin{definition}
