...
\begin{document}
    ...
    \end{theorem}
    \begin{proof}\side[-0.6in]{That is, $F$ looks like a power map $z\mapsto z^m$ locally around $p$.}
        Let $\tpl{U_2,\phi_2}$ be a chart of $Y$ centered at $F\l(p\r)$ and consider any chart $\tpl{V,\psi}$ of $X$ centered at $p$. Then the function $T\coloneqq\phi_2\circ F\circ\psi^{-1}$ is holomorphic, so it admits a power series representation $T\l(w\r)=\sum_{i=0}^{\infty}c_iw^i$ for all $w\in\psi\l(V\r)$. Note that $T\l(0\r)=\phi_2\l(F\l(p\r)\r)=0$, so $c_0=0$. Let $m\geq1$ be the smallest integer such that $c_m\neq0$, so
        \begin{equation*}
            T\l(w\r)=\sum_{i=m}^{\infty}c_iw^i=w^m\sum_{i=0}^{\infty}c_{i-m}w^i\eqqcolon w^mS\l(w\r).
        \end{equation*}
        Then $S$ is holomorphic at $0$ with $S\l(0\r)=c_m\neq0$, so there is a function $R$ holomorphic on some neighborhood $W$ of $0$ such that $\l(R\l(w\r)\r)^m=S\l(w\r)$ for all $w\in W$. Thus $T\l(w\r)=\l(wR\l(w\r)\r)^m$, so set $\eta\l(w\r)\coloneqq wR\l(w\r)$ for all $w\in W$. Note that $\eta'\l(0\r)=R\l(0\r)\neq0$, so $\eta$ is invertible on some neighborhood $W'\subseteq W$ of $0$. Set $U_1\coloneqq\psi^{-1}\l(W'\r)$ and $\phi_1\coloneqq\eta\circ\psi$. Then $\l(U_1,\phi_1\r)$ is a chart of $X$ centered at $p$ such that
        \begin{equation*}
            \l(\phi_2\circ F\circ\phi_1^{-1}\r)\l(z\r)=\l(\phi_2\circ F\circ\psi^{-1}\circ\eta^{-1}\r)\l(z\r)=T\l(\eta^{-1}\l(z\r)\r)=\l[\eta\l(\eta^{-1}\l(z\r)\r)\r]^m=z^m
        \end{equation*}
