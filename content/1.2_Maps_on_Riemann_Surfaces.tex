...
\begin{document}
    ...
    \side[-0.57in]{That is, $F$ looks like a power map $z\mapsto z^m$ locally around $p$. Since such an integer $m\geq1$ is unique, we call it the \ul{multiplicity of $F$ at $p$}, denoted $\mult_p\!\l(F\r)$.\\\ \\
        This theorem also give easy proofs of some elementary properties of holomorphic maps, which we collect here; see \cite{miranda} for details. Throughout, $X$ and $Y$ are Riemann surfaces and $F:X\to Y$ is a non-constant holomorphic map.
        \begin{itemize}
