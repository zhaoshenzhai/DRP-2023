...
\begin{document}
    Surprisingly, computing the moduli space for the torus $T^2$ is rather easy and almost no machinery is needed. We compute it in Section \ref{sec:moduli_space_of_torus} and devote the rest of the chapter to computing the moduli space for the sphere, $S^2$.
    \section{Case for $g=0$ and $g=1$}
    \subsection{Moduli Space of $T^2$}\label{sec:moduli_space_of_torus}
    ...
    \begin{proof}
        ...
        \end{itemize}
        For the forward direction, let $\phi:X_\tau\to X_{\tau'}$ be a biholomorphism, which lifts\side{This is a standard result in algebraic topology. For a proof, see \cite[][Theorem 3.4]{i&t}.} to a biholomorphic mapping $\tilde{\phi}:\C\to\C$ such that
        \begin{equation*}
            \begin{tikzcd}
