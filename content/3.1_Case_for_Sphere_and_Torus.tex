...
\begin{document}
    For any genus $g$, we let $\mc{M}_g$ denote the \textit{moduli space} of Riemann surfaces of genus $g$; that is\side{This defines $\mc{M}_g$ as a set, but it turns out that they can all be equipped with a natural complex structure.}, the set of all Riemann surfaces of genus $g$ up to biholomorphism. Using the language and machinery developed in Chapters 1 and 2, we compute $\mc{M}_0$ and $\mc{M}_1$, which are the moduli spaces of the sphere $S^2$ and the torus $T^2$, respectively.\\\ \\
