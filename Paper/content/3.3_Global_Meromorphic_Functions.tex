...
\begin{document}
    ...
    \begin{proof}
        ...
        \end{equation*}
        We need to show that this integral converges and depends differentiably on $z$. Since $g$ is compactly supported, the integrand only has a pole at $0$ and so it suffices to show that the integral over a disk $D_\epsilon\coloneqq\bar{B_\epsilon}\coloneqq\bar{B\l(0,\epsilon\r)}$ converges. Indeed, we change\side{Formally, we appeal to Proposition \ref{3.1:prp:change_of_variables_in_C}.} to polar coordinates to see that
        \begin{equation*}
        ...
        \end{equation*}
        which is convergent\side{Here, we use the fact that $g$ is bounded.}. Now, to show that $f\in\DIFF\l(\C\r)$, we expand the definition of $f$ into
        \begin{equation*}
        ...
        \end{equation*}
        Using $\d=\partial+\bar{\partial}$ and expanding the definitions, we have that\side[0.9in]{Use the Product Rule and that $1/\zeta$ is holomorphic away from $0$.}
        \begin{equation*}
            \begin{aligned}
        ...
        \end{equation*}
        This integral can be calculated in polar coordinates as $\zeta=\epsilon e^{i\theta}$ for $0\leq\theta<2\pi$, so\side[0.3in]{$\d\zeta=\frac{\partial\zeta}{\partial\theta}\,\d\theta=\epsilon ie^{i\theta}\d\theta$.}
        \begin{equation*}
        ...
        Now, for the general case, we consider an increasing sequence of radii $\l\{R_n\r\}$ such that $R_n\to\infty$ and their associated balls $B_n\coloneqq B\l(0,R_n\r)$. For all $n$, there exists\side{For instance, take bump functions.} a function $\psi_n\in\DIFF\l(\C\r)$ such that $\Supp\l(\psi_n\r)\subseteq B_{n+1}$ and $\l.\psi_n\r|_{B_n}=1$. Extending $\psi_ng$ by zero outside $B_{n+1}$, they become differentiable functions in $\C$ with compact supports and hence $\bar{\partial}f_n=\psi_ng\,\d\bar{z}$ for some $f_n\in\DIFF\l(\C\r)$. We shall inductively construct a new sequence $\big\{\tilde{f}_n\big\}$ of differentiable functions on $\C$ such that
        \begin{enumerate}
            \item $\bar{\partial}\tilde{f}_n=g\,\d\bar{z}$ on $B_n$ and
                \vspace{-0.05in}
            \item $\big\|\tilde{f}_{n+1}-\tilde{f}_n\big\|_{B_n}\!\leq 2^{-n}$\side{$\l\|f\r\|_K\coloneqq\sup_{x\in K}\l|f\l(x\r)\r|$ is the supremum norm.}.
        \end{enumerate}
        ...
        \end{equation*}
        on $B_n$, so the function $f_{n+1}-\tilde{f}_n$ is holomorphic on $B_n$. Thus there exists a polynomial $p\in\C\l[z\r]$\side{Say, some Taylor polynomial.} such that
        \begin{equation*}
    ...
    \end{proof}
    \begin{remark}
        This theorem (which we call \textit{Dolbeault's Theorem}) is a special case of the $\bar{\partial}$-Poincaré Lemma. Indeed, we can reformulate the theorem by saying that the sequence of sheaves\side[0.3in]{Here, $\iota$ is the inclusion sheaf morphism.}
        \begin{equation*}
            \begin{tikzcd}
    ...
    \begin{remark}
        ...
        \begin{itemize}
            \item The dimension $g\coloneqq\dim_\C\check{H}^1\!\l(X,\HOLO\r)$ is finite\side{See \cite[][Section 14]{forster} for a proof.} and is referred to as the \ul{genus of $X$}. The above theorem states that $\RS$ has genus $0$.
                \vspace{-0.05in}
