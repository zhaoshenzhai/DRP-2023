...
\begin{document}
    ...
    \end{definition}
    \side[-0.54in]{To give a complex structure $\mf{A}$ to $X$, it suffices to give $X$ a complex atlas since it extends to a \textit{unique} complex structure.}
    \begin{definition}
        A \uldef{Riemann surface} is a connected $1$-dimensional complex manifold.
    \end{definition}
    \begin{example}\label{1.1:exa:C_and_H}\side[-0.15in]{Every Riemann surface can be regarded as a (connected) $2$-dimensional real manifold by $\textrm{`}$forgetting$\textrm{'}$ its complex structure.}
        ...
        \begin{itemize}
            ...
            % \item The \ul{upper-half plane} $\H\coloneqq\l\{z\in\C\mid\Im z>0\r\}$ can be given a complex structure by choosing the chart map $\phi:\H\to\C:x+iy\mapsto x+i\log y$. This example is of interest since, if we define a conformal structure on $\H$ by pulling back the Euclidean metric on $\C$, the upper-half plane $\H$ becomes the \textit{Poincaré half-plane model} of the hyperbolic plane.\exqed
