...
\begin{document}
    ...
    \begin{example}[Biholomorphisms between Riemann spheres]\label{1.2:exa:biholomorphisms_between_Riemann_spheres}
        Let $\C_\infty$, $S^2$, and $\P^1$ denote the three constructions for the Riemann sphere $\RS$ presented in Examples \ref{1.1:exa:one_point_compactification_of_C}, \ref{1.1:exa:stereographic_projection}, and \ref{1.1:exa:complex_projective_line}, respectively. We claim that the maps\footnote{Take $G\l(0,0,1\r)\coloneqq\infty$.}
        \begin{equation*}
    ...
    \end{definition}\footnote{As in Example \ref{1.2:exa:elementary_holomorphic_functions}, if $f,g:W\to\C$ are both meromorphic at $p$, then so are $f\pm g$ and $f\cdot g$. If $g$ is not identically $0$, then so is $f/g$. This makes the set $\MERO\l(W\r)$ of all meromorphic functions $f:W\to\C$ into a $\C$-algebra.}
    \begin{remark}
