...
\begin{document}
    We develop the basics of covering space theory. More specifically, Section \ref{CS:sec:covering_maps_degree} develops the notion of the \textit{degree} to derive a criterion for a compact Riemann surface $X$ to be biholomorphic to the Riemann sphere $\RS$, and Section \ref{CS:sec:covering_transformations} derives the \textit{Lifting Criterion} from algebraic topology. Those tools will be used in Sections \ref{MS:sec:moduli_space_of_sphere} and \ref{MS:cor:moduli_space_torus} to compute the moduli spaces of $S^2$ and $T^2$, respectively. This chapter requires some background on homotopies of curves, for which we refer the reader to \cite[][Section 1.1]{hatcher}. Section \ref{CS:sec:covering_transformations} roughly follows \cite[][Section 1.3.1]{hatcher}, so the reader will benefit from reading it concurrently.
    ...
    We first gather some basic results on the theory of covering spaces from topology. Throughout this section and the next, let $E$ and $X$ be locally-compact topological spaces.
    \begin{definition}
        A map $\pi:E\to X$ is said to be a \uldef{covering map} if every point $p\in X$ admits a neighborhood $U$ such that $\pi^{-1}\l(U\r)=\coprod_{j\in J}V_j$ where $V_j$ are disjoint open sets in $E$, each homeomorphic to $U$ via $\l.\pi\r|_{V_j}$. In this case, we say that $U$ is \uldef{covered evenly by $\pi$} and that $E$ is a \uldef{covering space of $X$}.
    \end{definition}
    \begin{proposition}\label{CS:prp:fiber_cardinalities_coincide}
        Let $\pi:E\to X$ be a covering map. If $X$ is connected, then, for any $p_1,p_2\in X$, the fibers $\pi^{-1}\!\l(p_1\r)$ and $\pi^{-1}\!\l(p_2\r)$ are equinumerous.
    \end{proposition}
    \begin{proof}
        Let $\l\{U_i\r\}$ be an evenly-covered open cover of $X$, so $\pi^{-1}\!\l(U_i\r)=\coprod_{j\in J}V_{ij}$ for disjoint open sets $V_{ij}\subseteq E$ where each $\l.\pi\r|_{V_{ij}}:V_{ij}\to U_i$ is a homeomorphism. Fix $x_0\in X$, say $x_0\in U_0$, and consider the set $S\coloneqq\l\{x\in X\mid\l|\pi^{-1}\!\l(x\r)\r|=\l|\pi^{-1}\!\l(x_0\r)\r|\r\}$. We claim that $S$ is both open and closed, so $S=X$ and we are done.
        \begin{itemize}
            \item Take $x\in S$, say $x\in U_i$ for some $i\in I$. For all $x'\in U_i$, the set $\pi^{-1}\!\l(x'\r)\cap V_{ij}$ is a singleton for all $j\in J$, so $\l|\pi^{-1}\!\l(x'\r)\r|=\l|J\r|$. Thus $x'\in S$, so $U_i\subseteq S$.
            \item Take $x\in X\comp S$, so $\l|\pi^{-1}\!\l(x\r)\r|\neq\l|\pi^{-1}\!\l(x_0\r)\r|$ and hence $x\not\in U_0$. Thus $x\in U_i$ for some $i\neq0$, and for all $x'\in U_i$, the fiber $\pi^{-1}\!\l(x'\r)$ is not equinumerous with $\pi^{-1}\!\l(x_0\r)$. Thus $U_i\subseteq X\comp S$.\qed
        \end{itemize}
