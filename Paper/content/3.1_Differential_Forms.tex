...
\begin{document}
    ...
    \end{remark}
    \begin{example}
        For $f\in\DIFF\l(W\r)$, the form $\d f$ (resp. $\partial f$, $\bar{\partial}f$) is a differentiable (resp. type $\l(1,0\r)$, type $\l(0,1\r)$) $1$-form on $W$. Thus we have the map $\d:\DIFF\l(W\r)\to\DIFF^{\l(1\r)}\!\l(W\r)$, and similarly the maps $\partial$ and $\bar{\partial}$, called the \ul{exterior derivatives on $\DIFF\l(W\r)$}. They are studied in the next section.\exqed
    \end{example}
    \subsection{Differential $2$-forms and Exterior Differentiation}
    Define\side{For an in-depth discussion of the tensor product, see \cite[][Chapter 8.2]{aluffi}.} the \textit{exterior power} $\Lambda^2V$ of a $\C$-vector space $V$ as the quotient of the tensor product $V\otimes V$ by the ideal $\mf{a}\coloneqq\ideal{v\otimes v\mid v\in V}$. For completeness, we very briefly define $V\otimes V$.
    \begin{definition}
    ...
    \end{definition}
    \side[-0.92in]{Here, $j:V\times V\to F$ is a function making $\tpl{F,j}$ satisfy the universal property of the free vector space.}
    \vspace{-0.05in}
    \begin{remark}
        Let $V$ be a $\C$-vector space. For all $v_1,v_2\in V$, define $v_1\wedge v_2\in\Lambda^2V$ by the equivalence class of $v\otimes v$ modulo $\mf{a}$. It is then immediate from the definition of $V\otimes V$ that
        \begin{equation*}
        ...
        \begin{equation*}
            ...
            \end{aligned}
        \end{equation*}
        so $v_1\wedge v_2=-\l(v_2\wedge v_1\r)$ for all $v_1,v_2\in V$. Finally, if $\l\{e_i\r\}$ is a basis for $V$, then\side{For a proof, see \cite[][Proposition 12.8]{lee}.} $\l\{e_i\otimes e_j\r\}$ is a basis for $V\otimes V$. From this, we see that $\l\{e_i\wedge e_j\r\}_{i<j}$ is a basis for $\Lambda^2V$.\exqed
    \end{remark}
    \begin{remark}
